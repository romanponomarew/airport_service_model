...
1 версия модели:
    ...
    .1 внешний склад деталей(Производство запчастей)
    .Поиск деталей на складе и оформление заказа с помощью стандартных технологий(иерархическая структура)
    .Не известно сколько деталей находится в данный момент на производстве,
                    поэтому поиск и заказ запчвстей занимает большее время.
"""

import pygame
import simpy
from environment_standard_structure import (PyGameEnvironment, FrameRenderer)
import time
import itertools
import random
import json
...
class Truck:
    ...
    def __init__(self, env):
        ...
        self.x = 720  # Изначальное положение центра картинки(Склад)
        self.y = 330
